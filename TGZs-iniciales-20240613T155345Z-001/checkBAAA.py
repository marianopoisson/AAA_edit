...
afffile="BAAA_affiliations_20240801.txt"
...
def clean_keywords(text):
    ...
    if not (' --- ' in text):
      print('  * error separador keywords')
      return(0)
    ...
    for i,t in enumerate(text):
        ...
        if not (key in keywordList):
          print('  bad keyword: %s ' % key,'yellow')
          try:
            sugg =difflib.get_close_matches(r'{}'.format(key), keywordList)[0]
          except:
            sugg = 'NO MATCH'
          print('  suggestion : '+sugg,'yellow')
          print('')
...
def clean_title(title,subtitle):
    ...
    if (subtitle):
      ...
      title = title.replace('  ',' ')
      print('  Warning! tiene subtitulo %s ' % title)
