...
\title{Medidor de seeing del Cerro Burek (CASLEO)
}
...
\author{P.F. Pereyra\inst{1}, J.L. Aballay\inst{1}, G.L. Fern\'andez\inst{1}, M.A. Gim\'enez\inst{1}, J.D. Pinto\inst{1}, B.G. Giuliani\inst{1}, O.A. Collado\inst{1}, W.E. Alvar\'ez\inst{1} \& L.A. Mammana\inst{1}
}
...
\abstract{
Para caracterizar un sitio astronómico es fundamental medir la visibilidad astronómica (seeing), la cual se ve afectada por la turbulencia atmosférica. El personal técnico del CASLEO desarrolló un instrumento para medir el seeing utilizando un DIMM (Differential Image Motion Monitor). Este dispositivo cuenta con un sistema automático que lo protege de las inclemencias del tiempo, basado en un medidor de nubes, y un sistema manual que permite desconectar eléctricamente el instrumento durante tormentas eléctricas. Los datos generados por el medidor son accesibles a través de Ethernet y se muestran en línea en la página del Instituto. El instrumento ha sido validado por personal especializado del Cerro Tololo Inter-American Observatory (CTIO). Se cuenta con mediciones de seeing desde 2018. En este trabajo se presentan resultados preliminares.
}
...
\keywords{site testing --- atmospheric effects --- instrumentation: miscellaneous
}
...
\contact{ppereyra@casleo.gov.ar} 
