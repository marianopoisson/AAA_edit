...
thematicarea = {'1':"SH. Sol y Heliosfera",
                '2':"SSE. Sistemas Solar y Extrasolares",
                '3':"AE. Astrof\\'isica Estelar",
                '4':"SE. Sistemas Estelares",
                '5':"MI. Medio Interestelar",
                '6':"EG. Estructura Gal\\'actica",
                '7':"AEC. Astrof\\'isica Extragal\\'actica y Cosmolog\\'ia",
                '8':"OCPAE. Objetos Compactos y Procesos de Altas Energ\\'ias",
                '9':"ICSA. Instrumentaci\\'on y Caracterizaci\\'on de Sitios Astron\\'omicos",
               '10':"AGE. Astrometr\\'ia y Geodesia Espacial",
               '11':"ASOC. Astronom\\'ia y Sociedad",
               '12':"O. Otros"}
...
def write_sessions(db):
  order = ["Mesa redonda",
           ...
           "AE. Astrof\\'isica Estelar",
           "SE. Sistemas Estelares",
           "MI. Medio Interestelar",
           "EG. Estructura Gal\\'actica",
           "AEC. Astrof\\'isica Extragal\\'actica y Cosmolog\\'ia",
           "OCPAE. Objetos Compactos y Procesos de Altas Energ\\'ias",
           "AGE. Astrometr\\'ia y Geodesia Espacial",
           "ICSA. Instrumentaci\\'on y Caracterizaci\\'on de Sitios Astron\\'omicos",
           "ASOC. Astronom\\'ia y Sociedad",
           "O. Otros"]
  ...
  s = 0  # Contador de filas procesadas
  processed_rows = set()  # Conjunto para rastrear filas procesadas

  # Procesar la primera categoría de "Mesa redonda"
  for i in order[:1]:
    index = db['contribtype'] == i
    s += sum(index)
    processed_rows.update(db.index[index])
    for k in db.index[index]:
      f.write("\n\\session{" + i + "}")
      f.write("\n\\paperid{%s}{%s}" % (k, k))

  # Procesar las demás categorías
  for i in order[1:]:
    f.write("\n\\session{" + i + "}")
    for j in suborder:
  ...
  unprocessed_rows = set(db.index) - processed_rows
  if unprocessed_rows:
    print(f"Advertencia: No se procesaron las siguientes filas: {unprocessed_rows}")

  # Asegurarse de que el número total de filas procesadas coincide con el total de filas en db
  assert s == db.shape[0], f"Error: Se procesaron {s} filas, pero db tiene {db.shape[0]} filas."
